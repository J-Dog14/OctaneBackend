...
async function doEmitReportLinks(jobId: string, runnerId: string, reportDir: string, jobStartTime?: number): Promise<void> {
  try {
    ...
    // Only emit reports for PDFs created or modified during this run.
    // This prevents the entire historical reports folder from being emitted after every run.
    const recentPdfs: string[] = [];
    if (jobStartTime) {
      for (const filename of pdfs) {
        try {
          const s = await stat(path.join(reportDir, filename));
          if (s.mtimeMs >= jobStartTime - 2000) recentPdfs.push(filename); // 2s buffer for clock skew
        } catch { /* skip unreadable files */ }
      }
    } else {
      recentPdfs.push(...pdfs); // fallback: no start time, emit all
    }
